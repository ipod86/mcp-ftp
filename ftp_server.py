#!/usr/bin/env python3
"""MCP server providing FTP / FTPS / SFTP access for Claude Code.

Config:   /etc/mcp-ftp/ftp_config.ini  (owner: mcp-ftp, chmod 600)
Exchange: /var/lib/mcp-ftp/exchange    (chmod 770, group mcp-ftp)
...
def _load_config() -> configparser.ConfigParser:
    ...
    if not CONFIG_PATH.exists():
        raise FileNotFoundError(
            ...
            "Run manual_root_setup.sh, then fill in credentials:\n"
    ...
    if conn_type == "sftp":
        if not _PARAMIKO:
            raise ImportError(
                ...
                "Install: sudo /opt/mcp-ftp/venv/bin/pip install paramiko"
            )
    ...
    if conn_type in ("ftp", "ftps"):
        ...
        try:
            ...
            raise PermissionError(
                f"Login to '{server_name}' failed — check username/password in {CONFIG_PATH}\n"
...
def _resolve_exchange(local_path: str) -> Path:
    ...
    try:
        ...
        raise ValueError(
            f"Path must be inside the exchange directory ({EXCHANGE_DIR}). Got: {local_path}"
        )
...
@mcp.tool()
def upload_directory(server_name: str, local_dir: str, remote_path: str) -> str:
...
def download_directory(server_name: str, remote_path: str, local_dir: str = "") -> str:
    """Recursively download a remote directory into the exchange directory.

    Args:
        server_name: Name of the server as defined in the config
        remote_path: Directory to download on the server
        local_dir:   Destination inside the exchange directory (relative or empty).
...
@mcp.tool()
def sync_directory(server_name: str, local_dir: str, remote_path: str) -> str:
    """Upload only new or changed files to the server (size-based comparison).
    ...
    with _connect(server_name) as conn:
        ...
        for local_file in sorted(local_base.rglob("*")):
            ...
            conn.upload(local_file, remote)
            uploaded += 1
    return f"Synced {local_dir} -> {remote_path}: {uploaded} uploaded, {skipped} unchanged"
...
def read_text_file(server_name: str, remote_path: str, max_kb: int = 64) -> str:
    ...
    Useful for config files, logs, or small source files without downloading first.
...
def write_text_file(server_name: str, remote_path: str, content: str) -> str:
    ...
    No exchange directory needed — content is passed as a string directly.
...
def copy_file(server_name: str, from_path: str, to_path: str) -> str:
    ...
    The file is buffered in memory and re-uploaded — no exchange directory needed.
